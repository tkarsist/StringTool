...
public class StringToolMTOld {
	...
	public static void main(String[] args) throws IOException {
		...
		for (String name: map.keySet()){
			counter+=1;
			if(1.0*counter/map.size()*100>(completionStatus+9)){
				completionStatus+=10;
				System.out.print(completionStatus+"% ");
			}
			String key =name.toString();
			List<Future<ArrayList<String>>> futureList = new ArrayList<Future<ArrayList<String>>>();

			for (String name2: map.keySet()){
				String key2 =name2.toString();
				if(!key.equals(key2)){
					Callable<ArrayList<String>> worker = new StrCompareCallableOld(key,key2,compareAlgorithm, similarPercent, wordSwappingDepth);
					Future<ArrayList<String>> submit = executor.submit(worker);
					futureList.add(submit);
				}

			}
			for (Future<ArrayList<String>> future : futureList) {
				try {
					ArrayList<String> temp=new ArrayList<String>();
					temp=future.get();

					if(temp.size()==2){
						String keyZ=temp.get(0)+" |"+map.get(temp.get(0)).toString()+"|";
						String valueZ=temp.get(1)+" |"+map.get(temp.get(1)).toString()+"|";
						if(!keyZ.equals(valueZ)){
							ArrayList<String> duplikaatit = new ArrayList<String>();
							if(duplicateMap.containsKey(keyZ))
								duplikaatit = duplicateMap.get(keyZ);
							duplikaatit.add(valueZ);
							duplicateMap.put(keyZ, duplikaatit);
						}
